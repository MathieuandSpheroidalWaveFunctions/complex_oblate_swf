...
!  determine first kind oblate angular functions
              ...
              call s1leg(l,m,cc,iopang,iopnorm,barg,narg,lims1,ndec,nex, &
                ...
                nads(li,jarg)=0
                if(iopang.eq.2) nads(li,jarg)=naccds(jarg)
if (debug) then
...
if (output) then
                if(iopang.eq.1) write(30,1440) barg(jarg),s1c(jarg),is1e(jarg),naccs(jarg)
                if(iopang.eq.2) write(30,1450) barg(jarg),s1c(jarg),is1e(jarg),s1dc(jarg), &
